...
<head>
    ...
    <title>{{ isset($user) ? 'Edit User' : 'Sign Up' }} | ItensFlow</title>
...
<body class="bg-gradient-glow">
    <div class="min-h-screen w-screen flex flex-col justify-center items-center py-8 px-4">

        {{-- Tombol kembali jika sedang dalam mode Edit User --}}
        @if(isset($user))
            <a href="{{ url('/users') }}" class="fixed top-6 left-6 flex items-center gap-2 text-xs font-semibold text-gray-400 hover:text-[#C7FF3D] transition-colors">
                <i class="fa-solid fa-arrow-left"></i> Kembali ke Daftar Users
            </a>
        @endif
        ...
        <div class="w-full max-w-[370px] bg-[#151515] border border-[#303030] rounded-[18px] py-6 px-6 flex flex-col items-center shadow-2xl">
            <div class="text-center flex flex-col items-center mb-6">
                ...
                <h2 class="text-lg font-bold text-white tracking-wide pb-4">ItensFlow</h2>
                <h1 class="text-xl font-bold text-white mb-1.5">{{ isset($user) ? 'Edit Account' : 'Create Account' }}</h1>
                <p class="text-xs text-white/80">{{ isset($user) ? 'Perbarui data pengguna di bawah ini' : 'Sign up to start managing your projects' }}</p>
            </div>
            ...
            <div class="w-full">
                ...
                <form class="w-full" method="POST" action="{{ isset($user) ? url('/users/'.$user->id) : url('/users') }}">
                    @csrf
                    @if(isset($user))
                        @method('PUT')
                    @endif
                    ...
                    <div class="mb-3.5 w-full">
                        <label class="block text-[10px] font-semibold text-white tracking-widest uppercase mb-1.5">Nama Lengkap</label>
                    ...
                    <div class="mb-5 w-full">
                        ...
                        @if(isset($user))
                            <span class="text-[10px] text-gray-500 mt-1 block">*Kosongkan jika tidak ingin mengubah password</span>
                        @endif
                    </div>
                ...
                @if(!isset($user))
                    <p class="text-xs text-white/80 text-center">Already have an account? 
                        <a href="{{ url('/login') }}" class="text-[#C7FF3D] font-semibold hover:opacity-75">Sign In</a>
                    </p>
                @endif
            </div>
        </div>
